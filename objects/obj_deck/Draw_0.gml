...
if(ds_list_size(card_list)>0){
	...
	if(string_pos("human",string_lower(top_card))!=0){
		if(player_owner="me"){
			...
			draw_sprite_ext(spr_human_card_back,0,room_width*.1,sprite_get_width(spr_human_card_back)/1.5,.5,.5,0,c_white,1);
		}
	}
		...
		if(player_owner="me"){
			...
			draw_sprite_ext(spr_skeleton_card_back,0,room_width*.1,sprite_get_width(spr_human_card_back)/1.5,.5,.5,0,c_white,1);
		}
	}
}
